...
void RegistersModel::setRegisters(const std::vector<binja::rewind::core::model::RegisterValue>& regs) {
  beginResetModel();
  m_registers = regs;
  endResetModel();
}
...
void RegistersView::setRegisters(const std::vector<binja::rewind::core::model::RegisterValue>& regs) {
  m_model->setRegisters(regs);
}
