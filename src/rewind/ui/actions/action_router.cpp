...
bool is_rewind_action(const QString& name) { return binja::rewind::ui::action_id_from_name(name).has_value(); }

binja::rewind::ui::RewindSidebarWidget* active_rewind_widget(UIContext* context) {
  if (!context) {
    return nullptr;
  }
  ...
  return dynamic_cast<binja::rewind::ui::RewindSidebarWidget*>(widget);
}
...
UIActionContext build_action_context(binja::rewind::ui::RewindSidebarWidget* widget) {
  UIActionContext ctx{};
  if (!widget) {
    return ctx;
  }
...
public:
  bool eventFilter(QObject*, QEvent* event) override {
    ...
    for (const auto& spec : binja::rewind::ui::action_specs()) {
      if (!binja::rewind::ui::is_reverse_action(spec.id)) {
        continue;
      }
      if (spec.key.count() == 0) {
        continue;
...
void RewindActionRouter::OnActionExecuted(
  ...
  QPointer<binja::rewind::ui::RewindSidebarWidget> guard(widget);
